...
class AdminPanelModel extends Models {
  ...
  */
  public function getSettings($setting){
      ...
      if(!empty($settings_data[0]->setting_data)){
  ...
  */
    ...
    $au_signature = nl2br($au_signature);
		// Update users table
		$query = $this->db->update(PREFIX.'users', array('userName' => $au_username, 'userFirstName' => $au_firstName, 'userLastName' => $au_lastName, 'userEmail' => $au_email, 'gender' => $au_gender, 'website' => $au_website, 'aboutme' => $au_aboutme, 'signature' => $au_signature), array('userID' => $au_id));
		if($query > 0){
			return true;
		}else{
			return false;
