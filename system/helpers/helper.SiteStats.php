...
class SiteStats
{
  ...
  */
  public static function log($username = null){
    ...
    $no_log_pages = array("Templates", "assets", "FSGBotAPI", "DedicatedServerAPI");
  ...
  /**
    ...
    if($when == "lastMonth"){$date = date('F Y', strtotime($date.' -1 month'));}
    ...
    if(!$authModel->getDeviceExists($userId,$os,$device,$browser,$city,$state,$country,$useragent)){
      ...
      if($authModel->addIntoDB('users_devices',array('userID'=>$userId,'os'=>$os,'device'=>$device,'browser'=>$browser,'city'=>$city,'state'=>$state,'country'=>$country,'useragent'=>$useragent,'ip'=>$ip))){
        ...
        if(!empty($site_mail_setting)){
          ...
          $username = \Helpers\CurrentUserData::getUserFullName($userId);
          $mail = new \Helpers\Mail();
          $mail->addAddress($email);
          $mail->setFrom(SITE_EMAIL, EMAIL_FROM_NAME);
          $mail->subject(SITE_TITLE. " - ".Lang::get("",'login_device_email_sub'));
          $body = \Helpers\PageFunctions::displayEmailHeader();
          $body .= Lang::get("",'login_new_device_email',array($username, SITE_TITLE));
          $body .= "<hr><b>".Lang::get("",'device_device', 'Members')."</b>";
          $body .= "<br>".$browser." - ".$os;
          $body .= "<hr><b>".Lang::get("",'device_location', 'Members')."</b>";
          $body .= "<br>".$city.", ".$state.", ".$country;
          $body .= Lang::get("",'login_device_footer_email',array(SITE_URL));
