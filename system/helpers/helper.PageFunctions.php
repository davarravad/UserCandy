...
class PageFunctions {
  ...
  public static function checkPageGroup($page_id = null, $group_id = null){
	...
	public static function systemPagePermission($u_id = null){
		...
		$get_page_groups = (!empty($get_page_id)) ? self::getPagesGroupsID($get_page_id[0]->id) : "";
		/** Check to see if page permission is public **/
		if(!empty($get_page_groups)){
	      ...
	      if(!empty($current_user_groups)){
			...
			if($allow_page == false){
				...
				if($u_id > 0){
					...
					Popups::pushError('You Do Not have permission to view that page!', '');
				}else{
					/** User Not logged in - send them to login page **/
					Popups::pushError('You Must be Logged In to view that page!', 'Login');
				}
			}
		}
	}
	...
	public static function displayTermsPrivacy($type = null){
		if($type == "Terms"){
			$tp_data = Lang::get("",'terms_updated', array(SITE_TITLE, SITE_URL));
		}else if($type == "Privacy"){
			$tp_data = Lang::get("",'privacy_updated', array(SITE_TITLE, SITE_URL));
		}
		if(!empty($tp_data)){
			return $tp_data;
		}
							...
							<td class=\"container\">
								<div class=\"content\">
									...
									<table role=\"presentation\" class=\"main\">
										<tr>
											<td class=\"td_header\">
												<h1 class=\"header-text\">".SITE_TITLE."</h1>
											</td>
										</tr>
	...
	*/
	public static function displayEmailFooter(){
		...
		$html_data = "
					...
					<center>";
					//$html_data .= "&copy; ".date("Y")." ".SITE_TITLE." ".Lang::get("",'uc_all_rights').".";
					// if($data['terms_enabled'] == true){
					// 	$html_data .= " <br> ";
					// 	$html_data .= "<a href='".SITE_URL."Terms'>".Lang::get("",'terms_title')."</a>";
					// }
