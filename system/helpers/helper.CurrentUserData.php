...
class CurrentUserData
{
  ...
  * Get Group members count
  */
  public static function getGroupMembers($where_id){
    self::$db = Database::get();
    $user_groups = self::$db->select("
        SELECT
          *
  ...
  */
   ...
   * Get current user's username from database
   */
  public static function getUserFullName($where_id){
    self::$db = Database::get();
      $data = self::$db->select("SELECT userFirstName, userLastName FROM ".PREFIX."users WHERE userID = :userID",
        array(':userID' => $where_id));
        (isset($data[0]->userFirstName)) ? $userFirstName = $data[0]->userFirstName : $userFirstName = "";
        (isset($data[0]->userLastName)) ? $userLastName = $data[0]->userLastName : $userLastName = "";
  ...
  self::$db = Database::get();
  	$data = self::$db->select("SELECT userEmail FROM ".PREFIX."users WHERE userID = :userID",
  		array(':userID' => $where_id));
      (isset($data[0]->userEmail)) ? $email = $data[0]->userEmail : $email = "";
  	return $email;
  }
   ...
   * Get current user's email from database
   */
  public static function getUserNameEmail($userName){
    self::$db = Database::get();
      $data = self::$db->select("SELECT userEmail FROM ".PREFIX."users WHERE userName = :userName",
  ...
  /**
    ...
    self::$db = Database::get();
		$data = self::$db->select("SELECT signature FROM ".PREFIX."users WHERE userID = :userID",
			array(':userID' => $where_id));
        (isset($data[0]->signature)) ? $signature_data = \Helpers\BBCode::getHtml($data[0]->signature) : $signature_data = "";
